...
"""
No-pruning Baseline Script

This script runs the no-pruning baseline on GLUE benchmark tasks.
This method uses the full original vocabulary without any pruning or modification.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run no-pruning baseline on GLUE tasks"
    )
...
def main():
    """Run no-pruning baseline on specified GLUE tasks."""
    args = parse_args()
    ...
    for task in args.tasks:
        ...
        # Log task parameters
        logger.info(f"\n{'=' * 50}")
        logger.info(f"Running no-pruning baseline for task: {task}")
        ...
        try:
            ...
            logger.info(f"Successfully completed no-pruning baseline for task: {task}")
            
            # Find the log file for this task
            log_file = os.path.join(task_output_dir, f"{task}_no_pruning.log")
            # Try a pattern-based search as a fallback
            if not os.path.exists(log_file):
                pattern = f"{task}_no_pruning*.log"
            ...
            if os.path.exists(log_file):
                ...
                logger.warning(f"Tried looking for: {os.path.join(task_output_dir, f'{task}_no_pruning*.log')}")
                
        except subprocess.CalledProcessError as e:
            logger.error(f"Error running no-pruning baseline for task {task}: {e}")
            continue
