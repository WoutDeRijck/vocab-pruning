...
"""
Frequency-based Vocabulary Pruning Script

This script runs frequency-based vocabulary pruning on all GLUE benchmark tasks.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run frequency-based vocabulary pruning on GLUE tasks"
    )
    ...
    parser.add_argument(
        "--output_dir", 
        type=str, 
        default="./frequency_pruning_output",
...
def main():
    ...
    for task in args.tasks:
        ...
        # Log task parameters
        logger.info(f"\n{'=' * 50}")
        logger.info(f"Running frequency-based pruning for task: {task}")
        ...
        try:
            ...
            log_file = os.path.join(
                task_output_dir, 
                f"{task}_frequency_prune{int(task_params['prune_percent'])}.log"
            )
            ...
            if not os.path.exists(log_file):
                pattern = f"{task}_frequency_prune*.log"
            ...
            if os.path.exists(log_file):
                ...
                logger.warning(f"Tried looking for: {os.path.join(task_output_dir, f'{task}_frequency_prune*.log')}")
